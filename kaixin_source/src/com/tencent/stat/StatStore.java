...
{
  private static StatStore instance;
  ...
  void deleteEvents(List<StoredEvent> paramList)
  {
    try
    {
      ...
      this.handler.post(new Runnable(paramList)
      {
        public void run()
        {
          StatStore.this.directDeleteEvents(this.val$evs);
        }
      });
      return;
    }
  ...
  void loadEvents(int paramInt)
  {
    this.handler.post(new Runnable(paramInt)
    {
      public void run()
      {
        ...
        for (int j = StatConfig.getMaxLoadEventCount(); ; j = i)
        {
          ...
          StatDispatcher.getInstance().send(localArrayList1, new StatDispatchCallback(localArrayList2, j)
          {
            public void onDispatchFailure()
            {
              StatStore localStatStore = StatStore.this;
              localStatStore.numStoredEvents += this.val$loadNumber;
              StatStore.this.updateEvents(this.val$evs, 1);
            }

            public void onDispatchSuccess()
            {
              StatStore.this.deleteEvents(this.val$evs);
              if (StatStore.7.this.val$maxNumber == -1);
              for (int i = StatStore.7.this.val$maxNumber; ; i = StatStore.7.this.val$maxNumber - this.val$evs.size())
              {
                if ((i == -1) || (i > 0))
  ...
  void storeCfg(StatConfig.OnlineConfig paramOnlineConfig)
  {
    if (paramOnlineConfig == null)
      return;
    try
    {
      this.handler.post(new Runnable(paramOnlineConfig)
      {
        // ERROR //
        public void run()
        {
  ...
  void storeEvent(Event paramEvent, StatDispatchCallback paramStatDispatchCallback)
  {
    ...
    this.handler.post(new Runnable(paramEvent, paramStatDispatchCallback)
    {
      public void run()
      {
        StatStore.this.directStoreEvent(this.val$ev, this.val$cb);
      }
    });
  }

  void updateEvents(List<StoredEvent> paramList, int paramInt)
  {
    try
    {
      if (Thread.currentThread().getId() == this.handler.getLooper().getThread().getId())
      ...
      this.handler.post(new Runnable(paramList, paramInt)
      {
        public void run()
        {
          StatStore.this.directUpdateEvents(this.val$evs, this.val$status);
        }
      });
      return;
    }
