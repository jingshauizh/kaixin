...
public class TAuthView extends Activity
{
  ...
  private void returnResult()
  {
    ...
    if ((this.from_cmd != null) && (this.from_cmd.equals("send_store")))
      TencentOpenAPI.openid(this.mAccessToken, new Callback()
      {
        ...
        public void onFail(int paramInt, String paramString)
        {
          TAuthView.this.runOnUiThread(new Runnable(paramString)
          {
            public void run()
            {
              TDebug.msg(this.val$msg, TAuthView.this.getApplicationContext());
            }
          });
        }
        ...
        public void onSuccess(Object paramObject)
        {
          ...
          TencentOpenAPI2.sendStore(TAuthView.this.getBaseContext(), TAuthView.this.mAccessToken, TAuthView.this.cache_client_id, TAuthView.this.openidString, TAuthView.this.cache_target, localBundle, new Callback()
          {
            public void onCancel(int paramInt)
            {
            }
            ...
            public void onFail(int paramInt, String paramString)
            {
              TAuthView.this.runOnUiThread(new Runnable(paramInt, paramString)
              {
                public void run()
                {
                  Toast localToast = Toast.makeText(TAuthView.this.getBaseContext(), "分享失败，错误信息:" + this.val$ret + ", " + this.val$msg, 0);
            ...
            public void onSuccess(Object paramObject)
            {
              TAuthView.this.runOnUiThread(new Runnable()
              {
                public void run()
                {
                  Toast localToast = Toast.makeText(TAuthView.this.getBaseContext(), "成功分享到QQ空间！", 0);
                  localToast.setGravity(17, 0, 0);
                  localToast.show();
                }
              });
  ...
  protected void onCreate(Bundle paramBundle)
  {
    ...
    if (localBundle != null)
    {
      ...
      String str2 = localBundle.getString("scope");
      ...
      String str6 = localBundle.getString("target");
