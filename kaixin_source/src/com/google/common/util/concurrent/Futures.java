...
public final class Futures
{
  ...
  public static <V> void addCallback(ListenableFuture<V> paramListenableFuture, FutureCallback<? super V> paramFutureCallback, Executor paramExecutor)
  {
    ...
    paramListenableFuture.addListener(new Runnable(paramListenableFuture, paramFutureCallback)
    {
      public void run()
      {
        try
  ...
  public static <I, O> Future<O> lazyTransform(Future<I> paramFuture, Function<? super I, ? extends O> paramFunction)
  {
    ...
    return new Future(paramFuture, paramFunction)
    {
      ...
      {
        return applyTransformation(this.val$future.get(paramLong, paramTimeUnit));
      }
  ...
  {
    return JdkFutureAdapters.listenInPoolThread(paramFuture);
  }
  ...
  public static <V> UninterruptibleFuture<V> makeUninterruptible(Future<V> paramFuture)
  {
    ...
    return new UninterruptibleFuture(paramFuture)
    {
      ...
      public V get()
        throws ExecutionException
      {
        return Uninterruptibles.getUninterruptibly(this.val$future);
      }

      public V get(long paramLong, TimeUnit paramTimeUnit)
        throws TimeoutException, ExecutionException
      {
        return Uninterruptibles.getUninterruptibly(this.val$future, paramLong, paramTimeUnit);
      }
    ...
    implements Runnable
  {
    ...
    public void run()
    {
      try
      {
        ...
        localListenableFuture = (ListenableFuture)this.function.apply(localObject2);
      ...
      catch (ExecutionException localExecutionException)
      {
        ...
        return;
        localListenableFuture.addListener(new Runnable(localListenableFuture)
        {
          public void run()
          {
            try
            {
              Futures.ChainingListenableFuture.this.set(Uninterruptibles.getUninterruptibly(this.val$outputFuture));
              return;
            }
            catch (CancellationException localCancellationException)
            {
  ...
  private static class ListFuture<V> extends AbstractFuture<List<V>>
  {
    ...
    private void init(Executor paramExecutor)
    {
      ...
      while (true)
      {
        ...
        {
          ListenableFuture localListenableFuture = (ListenableFuture)localImmutableList.get(j);
          localListenableFuture.addListener(new Runnable(j, localListenableFuture)
          {
            public void run()
            {
              Futures.ListFuture.this.setOneValue(this.val$index, this.val$listenable);
            }
          }
          , paramExecutor);
        }
