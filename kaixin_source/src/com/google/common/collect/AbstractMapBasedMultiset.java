  ...
  implements Serializable
{
  ...
  private int removeAllOccurrences(@Nullable Object paramObject, Map<E, Count> paramMap)
  {
    Count localCount = (Count)paramMap.remove(paramObject);
    if (localCount == null)
      return 0;
    int i = localCount.getAndSet(0);
    this.size -= i;
    return i;
  }
  ...
  {
    return new Iterator(this.backingMap.entrySet().iterator())
    {
      ...
      public Multiset.Entry<E> next()
      {
        ...
        return new Multisets.AbstractEntry(localEntry)
        {
          public int getCount()
          {
            int i = ((Count)this.val$mapEntry.getValue()).get();
            if (i == 0)
            {
