...
{
  public static <K> boolean beyond(GeneralRange<K> paramGeneralRange, K paramK, BstSide paramBstSide)
  {
  ...
  private static <K, N extends BstNode<K, N>> int totalBeyondRangeToSide(BstAggregate<? super N> paramBstAggregate, GeneralRange<K> paramGeneralRange, BstSide paramBstSide, @Nullable N paramN)
  {
    int i = 0;
    while (paramN != null)
    {
      if (beyond(paramGeneralRange, paramN.getKey(), paramBstSide))
      {
        i = i + paramBstAggregate.entryValue(paramN) + paramBstAggregate.treeValue(paramN.childOrNull(paramBstSide));
  ...
  public static <K, N extends BstNode<K, N>> int totalInRange(BstAggregate<? super N> paramBstAggregate, GeneralRange<K> paramGeneralRange, @Nullable N paramN)
  {
    ...
    {
      return i;
      i = paramBstAggregate.treeValue(paramN);
      if (!paramGeneralRange.hasLowerBound())
        continue;
      i -= totalBeyondRangeToSide(paramBstAggregate, paramGeneralRange, BstSide.LEFT, paramN);
    }
    while (!paramGeneralRange.hasUpperBound());
    return i - totalBeyondRangeToSide(paramBstAggregate, paramGeneralRange, BstSide.RIGHT, paramN);
  }
}
