...
final class EANManufacturerOrgSupport
{
  ...
  {
    this.ranges.addElement(paramArrayOfInt);
    this.countryIdentifiers.addElement(paramString);
  }
  ...
  String lookupCountryIdentifier(String paramString)
  {
    ...
    for (int k = 0; ; k++)
    {
      ...
      {
        return null;
        arrayOfInt = (int[])this.ranges.elementAt(k);
        m = arrayOfInt[0];
      }
      while (i < m);
      if (arrayOfInt.length == 1);
      for (int n = m; i <= n; n = arrayOfInt[1])
        return (String)this.countryIdentifiers.elementAt(k);
    }
  }
}
