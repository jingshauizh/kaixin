...
public class FaceModel extends KXModel
{
  ...
  public HashMap<String, Bitmap> getAllFaceInfo()
  {
    ...
    for (int m = 0; ; m++)
    {
      if (m >= k)
      {
        ...
        label57: this.mAllFaceInfoMap.put((String)this.mStateStrings.get(j), (Bitmap)this.mStateFaceIcons.get(j));
        j++;
        break;
      }
      this.mAllFaceInfoMap.put((String)this.mEmojiCodes.get(m), (Bitmap)this.mEmojiIcons.get(m));
    }
  }
  ...
  public ArrayList<String> getAllFaceStringSortByLength()
  {
    ...
    if (this.mAllFaceStringsSortByLength == null)
    {
      ...
      localIterator1 = this.mStateStrings.iterator();
      if (localIterator1.hasNext())
        break label59;
      localIterator2 = this.mEmojiCodes.iterator();
    }
    while (true)
    {
      if (!localIterator2.hasNext())
      {
        ...
        label59: String str1 = (String)localIterator1.next();
        int i = this.mAllFaceStringsSortByLength.size();
        int j = 0;
        if (i > 0);
        label149: label151: for (int k = 0; ; k++)
        {
          j = 0;
          if (k >= i);
          while (true)
          {
            if (j != 0)
              break label149;
            this.mAllFaceStringsSortByLength.add(str1);
            break;
            if (str1.length() < ((String)this.mAllFaceStringsSortByLength.get(k)).length())
              break label151;
            this.mAllFaceStringsSortByLength.add(k, str1);
            j = 1;
          }
          break;
        }
      ...
      this.mAllFaceStringsSortByLength.add(0, str2);
    }
  }
